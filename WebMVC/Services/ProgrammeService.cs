...
public class ProgrammeService : IProgrammeService
{
    ...
    {
        var response = await _httpClient.PostAsJsonAsync($"{_baseUrl}/programme", programme);
        response.EnsureSuccessStatusCode();
        return await response.Content.ReadFromJsonAsync<Programme>();
    }

    public async Task<Programme> UpdateProgrammeAsync(int id, Programme programme)
    {
        var response = await _httpClient.PutAsJsonAsync($"{_baseUrl}/programme/{id}", programme);
        response.EnsureSuccessStatusCode();
        return await response.Content.ReadFromJsonAsync<Programme>();
    }
